...
myregister::myregister(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::myregister), _countdown(5)
{
    ui->setupUi(this);
    ...
    connect(ui->captcha_edit, &QLineEdit::editingFinished, this, [this](){
        checkVarifyValid();
    });
...
void myregister::AddTipErr(TipErr te, QString tips)
{
    _tip_errs[te] = tips;
    showTip(tips, false);
}

void myregister::DelTipErr(TipErr te)
{
    _tip_errs.remove(te);
    if(_tip_errs.empty()){
        ui->err_tip->clear();
...
bool myregister::checkUserValid()
{
...
bool myregister::checkEmailValid()
{
    //验证邮箱的地址正则表达式
    auto email = ui->email_edit->text();
    // 邮箱地址的正则表达式
...
bool myregister::checkPassValid()
{
...
bool myregister::checkVarifyValid()
{
    auto pass = ui->captcha_edit->text();
    if(pass.isEmpty()){
...
bool myregister::checkConfirmValid()
{
    auto pass = ui->pass_edit->text();
    auto confirm = ui->confirm_edit->text();
...
void myregister::on_return_btn_clicked()
{
    _countdown_timer->stop();
    emit sigSwitchLogin();
}

void myregister::on_cancel_btn_clicked()
{
    _countdown_timer->stop();
    emit sigSwitchLogin();
}
